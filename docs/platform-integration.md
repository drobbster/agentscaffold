...
<!-- Managed by AgentScaffold. Content inside this block is regenerated; edits here
     are overwritten. Everything OUTSIDE the markers is always preserved. To take
     ownership, add an HTML comment agentscaffold: managed=false near the top. -->
... generated routing ...
...
- If the file **exists without the markers** (you/your org already authored it), AgentScaffold **appends** a fresh managed block at the end and does not touch a single existing byte -- unless the file already carries a substantial share of the generated headings, in which case it refuses and names `--allow-append`.
- Anything you write **outside** the markers is preserved across every regeneration. To take full ownership, add an HTML comment `agentscaffold: managed=false` near the top. Deleting the markers is not ownership: the next generation will append one more block. A marker-less file that already looks generated refuses unless you pass `--allow-append`. `--force` still replaces the entire file.
...
`--force` always writes a `.bak` snapshot (`AGENTS.md.bak`, `testing.md.bak`, ...) of the previous content before overwriting, so a forced rewrite is still recoverable. `--allow-append` is different: it only appends a managed block. On a duplicated `AGENTS.md`, use `scaffold agents repair` (dry run; `--apply` writes). To pull later manual template updates, use `scaffold agents diff-manual`.
...
- `.cursor/rules/agentscaffold.mdc` (MCP-first tool routing policy + intent map).
  Cursor only loads `.mdc` rule files; a plain `.md` is ignored, so a stale one
  from an older run is removed rather than left to look authoritative.

### Rules Setup

```bash
scaffold agents generate    # Refresh the AGENTS.md routing block
scaffold agents cursor      # Generate .cursor/rules.md and .cursor/rules/agentscaffold.mdc
```

...
def _workspace_scope_discipline_lines() -> list[str]:
    return [
        ...
        "- To look at another project, pass `--project <name>`; to search across all",
        "  of them, pass `--all-projects` (federated results carry a `project`",
        "  provenance field -- always report which project a cross-project hit came",
        "  from).",

...
MANAGED_FALSE_SENTINEL = "<!-- agentscaffold: managed=false -->"
_HEADING_GUARD_RATIO = 0.5
_MANAGED_BLOCK_NOTE = (
    ...
    "Everything OUTSIDE the markers is always preserved. To take full ownership "
    "of this file, add an HTML comment `agentscaffold: managed=false` near the "
    "top. Deleting the markers is not ownership: the next generation will append "
    "one more block. -->"
)


class ManagedBlockAppendRefusedError(RuntimeError):
    """Marker-less target already looks generated; appending would duplicate it."""


def has_managed_false_sentinel(text: str) -> bool:
    """Return True when *text* opts out of every AgentScaffold generator."""
    return MANAGED_FALSE_SENTINEL in text


def markdown_h2_headings(text: str) -> list[str]:
    """Return `## ` heading lines from *text*, in order."""
    return [line.rstrip() for line in text.splitlines() if line.startswith("## ")]


def heading_overlap_ratio(existing: str, block: str) -> float:
    """Share of *block*'s `## ` headings that already appear in *existing*."""
    generated = set(markdown_h2_headings(block))
    if not generated:
        return 0.0
    present = set(markdown_h2_headings(existing))
    return len(generated & present) / len(generated)
...
def write_managed_block(
    ...
    *,
    ...
    - Marker-less file already carries a substantial share of the generated
      headings: refuse unless ``allow_append``. Raises
      :class:`ManagedBlockAppendRefusedError`. ``--force`` is not this escape.
    - File contains ``MANAGED_FALSE_SENTINEL``: do not write. Returns ``"skipped"``.
    ...
    ``"unchanged"``, ``"overwritten"``, ``"skipped"``.
    """
    if path.exists() and has_managed_false_sentinel(path.read_text()):
        return "skipped"
...
def _upsert_block(
    ...
    *,
    ...
    ``"block-updated"``, ``"unchanged"``, ``"overwritten"``, ``"skipped"``.
    """
    status, updated = _plan_block(
        path,
        block,
        begin_marker,
        end_marker,
...
def _plan_block(
    ...
    *,
    ...
    existing = path.read_text()
    if has_managed_false_sentinel(existing):
        return "skipped", None
    ...
    if (
        ...
        and heading_overlap_ratio(existing, block) >= _HEADING_GUARD_RATIO
    ):
        raise ManagedBlockAppendRefusedError(
            f"{path} already contains a substantial share of the generated "
            "headings. Pass --allow-append to append the managed block once. "
            "Do not use --force: that flag replaces the entire file."
        )
...
_GITIGNORE_BLOCK_NOTE = (
    ...
    "# everything OUTSIDE the markers is always preserved. To take full ownership,\n"
    "# add an HTML comment `agentscaffold: managed=false` near the top. Deleting\n"
    "# the markers is not ownership: the next generation will append one more block."
)
...
def write_gitignore_block(path: Path) -> str:
    ...
    The heading-overlap guard does not apply: this block has no ``## `` headings.
...
GUIDANCE_POINTER_BEGIN = "<!-- agentscaffold-guidance-pointer -->"
GUIDANCE_POINTER_TEXT = (
    "Routing guidance: `.cursor/rules/agentscaffold.mdc` "
    "(regenerated by `scaffold agents generate-all`)."
)
GUIDANCE_IGNORE_REMEDIATION = (
    "If `.cursor/` is ignored as a directory, a file-level negation cannot "
    "re-include the rule file. Use a directory-scoped pair instead:\n"
    "  .cursor/*\n"
    "  !.cursor/rules/"
)
...
def _workspace_relative_source(source: str) -> str:
    """Never emit an absolute home path in generated provenance."""
...
def generated_banner(*, generator: str, source: str, digest: str) -> str:
    """First-line ``@generated`` provenance (Plan 251). Paths are relative."""
    rel = _workspace_relative_source(source)
    return f"<!-- @generated by {generator} source={rel} sha256={digest} -->\n"


def prepend_generated_banner(
    content: str, *, generator: str, source: str, digest: str
) -> str:
    """Insert the banner after YAML frontmatter so ``.mdc`` first-line rules survive."""
    banner = generated_banner(generator=generator, source=source, digest=digest)
    if banner.strip() in content:
        return content
    if content.startswith("---"):
        ...
        if end != -1:
            ...
            if insert_at != -1:
                return content[: insert_at + 1] + banner + content[insert_at + 1 :]
...
def render_guidance_stamp(canonical_sha: str, source: str) -> str:
    ...
    """
    return f"<!-- {GUIDANCE_STAMP_KEY}: {canonical_sha} source: {_workspace_relative_source(source)} -->"
...
def stamp_guidance(content: str, canonical_sha: str, source: str) -> str:
    """Add ``@generated`` banner plus the trailing hash stamp.

    Banner sits after YAML frontmatter so Cursor's first-line ``.mdc``
    requirement is unchanged. The hash comment stays at the end for drift
    detection.
    """
    rel = _workspace_relative_source(source)
    headed = prepend_generated_banner(
        content,
        generator="scaffold agents generate-all",
        source=rel,
        digest=canonical_sha,
    )
    return f"{headed.rstrip()}\n\n{render_guidance_stamp(canonical_sha, rel)}\n"


def render_agents_guidance_pointer() -> str:
    """Create-if-absent pointer that lives outside managed markers."""
    return f"{GUIDANCE_POINTER_BEGIN}\n{GUIDANCE_POINTER_TEXT}\n"


def ensure_agents_guidance_pointer(path: Path) -> str:
    """Insert the AGENTS.md guidance pointer if it is not already present.

    Never writes inside the managed block. Human text around the markers
    survives regeneration. Returns ``created``, ``inserted``, ``unchanged``,
    or ``skipped``.
    """
    pointer = render_agents_guidance_pointer()
    if not path.exists():
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(f"# Agent Operating Rules\n\n{pointer}\n")
        return "created"
    text = path.read_text()
    if has_managed_false_sentinel(text):
        return "skipped"
    if GUIDANCE_POINTER_BEGIN in text:
        return "unchanged"
    begin = text.find(MANAGED_BLOCK_BEGIN)
    if begin >= 0:
        prefix, rest = text[:begin], text[begin:]
        if prefix and not prefix.endswith("\n"):
            prefix += "\n"
        if prefix and not prefix.endswith("\n\n"):
            prefix += "\n"
        path.write_text(f"{prefix}{pointer}\n{rest}")
        return "inserted"
    if text.startswith("#"):
        nl = text.find("\n")
        if nl == -1:
            ...
            path.write_text(f"{text[: nl + 1]}\n{pointer}\n{text[nl + 1 :]}")
        return "inserted"
    path.write_text(f"{pointer}\n{text}")
    return "inserted"


def is_git_ignored(repo_root: Path, relpath: str | Path) -> bool:
    """True when ``git check-ignore`` reports *relpath* as ignored."""
    try:
        proc = subprocess.run(
...
def ignored_guidance_files(project_root: Path) -> list[Path]:
    """Guidance copies (and AGENTS.md) that git will not track."""
    candidates = [project_root / rel for rel in GUIDANCE_COPY_RELPATHS]
    ...
    for path in candidates:
        ...
        try:
            ...
            rel = str(path)
        if is_git_ignored(project_root, rel):
            ignored.append(path)
    return ignored
